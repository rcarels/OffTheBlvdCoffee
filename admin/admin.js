...
if (eventForm) {
  eventForm.addEventListener("submit", async (event) => {
    ...
    const activeInput = eventForm.querySelector('[name="is_active"]');
    if (activeInput) {
      activeInput.remove();
    }
  ...
  eventsList.innerHTML = result.events
    .map(
      ...
      <article class="quote-card">
        ...
        <div class="actions">
          <button class="btn small" onclick='startEditEvent(${JSON.stringify(
            event.id
          )}, ${JSON.stringify(event.title || "")}, ${JSON.stringify(
        event.event_date || ""
      )}, ${JSON.stringify(event.location || "")}, ${JSON.stringify(
        event.description || ""
      )}, ${event.is_active ? 1 : 0})'>
            Edit
          </button>

          <button class="btn secondary small" onclick='toggleEvent(${JSON.stringify(
            event.id
          )}, ${JSON.stringify(event.title || "")}, ${JSON.stringify(
        event.event_date || ""
      )}, ${JSON.stringify(event.location || "")}, ${JSON.stringify(
